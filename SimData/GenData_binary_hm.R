...
# sd.e: outcome model noise
# sd.s: selection model noise
# sd.x: X2, X3 ~ N(0, sd.x^2)
# sd.y: Y's noise
# rho: covariance of sd.s and sd.e for selection
# _o: outcome
# _s: selection

GenData_binary_hm <- function(N,seed,alpha,theta,beta,delta,
                              ...
                              rho=0)
{
  ...
  if (delta == 0) {
    ...
    x1 <- ( c(cbind(1, x2) %*% alpha + noises[, 2]) ) > 0
    ...
    x1 <- ( c(cbind(1, x2) %*% alpha) + rnorm(N, mean = 0, sd = sd.o) ) > 0
