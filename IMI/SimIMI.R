...
SimIMI = function(scen,R,n,M,batch=0)
{
  ...
  for ( r in 1:R )
  {
    ...
    fit = IMI(d,M,midx,Xidx_s,Xidx_o,MImethod,n)
    Theta[,r] = fit$theta
    comm[r] = fit$comm
  }
...
IMI = function(d,M,midx,Xidx_s,Xidx_o,method,n)
{
  p_s <- length(Xidx_s)
  p_o <- length(Xidx_o)
  q = length(midx)
  ...
  for ( j in 1:q ) {
    if ( method[j] == "norm" ) {
      fit.imp[[j]] = HMML_LS(d,midx,Xidx_s,Xidx_o,n)
    } else {
      fit.imp[[j]] = HMML_Logit(d,midx,Xidx_s,Xidx_o,n)
    }
  }
        ...
        if (length(idx) != 0) {
          if ( method[j] == "norm" ) {
            alpha <- mvtnorm::rmvnorm(n = 1,
                                      mean = fit.imp[[j]]$beta[, k],
                                      sigma = fit.imp[[j]]$Vb[, , k])
            idx = ni[k] + which(miss[(ni[k]+1):ni[k+1],midx[j]])
  ...
  list(theta=theta,thetas=thetas,comm=comm)
}
