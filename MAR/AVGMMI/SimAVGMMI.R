...
AVGMMI = function(d,M,midx,method,n)
{
  ...
  {
    if ( method[j] == "norm" )
      fit.imp[[j]] = AVGMLS(d,midx[j],setdiff(1:(p+2),midx),n)
    else
      fit.imp[[j]] = AVGMLogit(d,midx[j],setdiff(1:(p),midx),n,c(0,d$alpha))  # 1:p for real data, 1:p+2 for simulation, lam = 0.000 for simulation, lam = 0.005 for real data
