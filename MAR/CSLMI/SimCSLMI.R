...
CSLMI = function(d,M,midx,method,n)
{
  ...
  {
    if ( method[j] == "norm" )
      fit.imp[[j]] = CSLLS(d,midx[j],setdiff(1:(p+2),midx),n)
    else
      fit.imp[[j]] = CSLLogit(d,midx[j],setdiff(1:(p),midx),n,c(0,d$alpha)) # changed c(0,d$alpha) to d$beta   # 1:p for real data, 1:p+2 for simulation, lam = 0.000 for simulation, lam = 0.005 for real data
  ...
  for ( m in 1:M )
  {
    for ( k in 1:K )
    {
      for ( j in 1:q )
      {
        ...
        {
          alpha = fit.imp[[j]]$beta + backsolve(fit.imp[[j]]$cfisher,rnorm(p+1))
          idx = ni[k] + which(miss[(ni[k]+1):ni[k+1],midx[j]])
